...
class BaseDataset(Dataset):
    ...
    def __getitem__(self, idx):
        pth = self.paths[idx]
        # img_raw = Image.open(pth).convert('RGB')
        img_raw = Image.fromarray(cv2.imread(pth))
        if self.transform:
            img = self.transform(img_raw)
            img1 = self.transform(img_raw)

        label = torch.tensor(self.labels[idx, :])
        out = {"image": img, "label": label, "path": pth, "index": idx, "image1": img1}

        if getattr(self, 'cluster_labels', None) is not None:
            out['cluster_label'] = self.cluster_labels[idx]
