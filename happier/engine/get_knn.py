...
def get_knn(references, queries, num_k, embeddings_come_from_same_source, with_faiss=False, hyp_c=0):
    with_faiss = lib.str_to_bool(os.getenv("WITH_FAISS", with_faiss))

    num_k += embeddings_come_from_same_source

    if hyp_c > 0:
        assert with_faiss==False

    if with_faiss:
        distances, indices = get_knn_faiss(references, queries, num_k)
    else:
        distances, indices = get_knn_torch(references, queries, num_k, hyp_c)
...
def get_knn_torch(references, queries, num_k, hyp_c):
    lib.LOGGER.debug("Computing k-nn with torch")

    # scores = queries @ references.t()

    scores = - torch.cdist(queries, references)
